...
demos_config = [
    {
        'path': 'basic_usage',
        'title': '基础使用',
        'description': fac.AntdParagraph(['使用参数', fac.AntdText('title', code=True), fac.AntdText('subTitle', code=True),'配置基础的结果提示信息'])
    },
    {
        'path': 'status_usage',
        'title': '不同的状态类型',
        'description': fac.AntdParagraph(['使用参数', fac.AntdText('status', code=True), '设置结果的状态类型，8种状态类型分别为：info、success、error、warning、404、403、500、loading'])
    },
    {
        'path': 'custom_icon',
        'title': '自定义图标',
        'description': fac.AntdParagraph(['使用', fac.AntdText('icon', code=True),'参数和',fac.AntdText("fac.AntdIcon()",code=True),'组件设置自定义图标'])
    },
    {
        'path': 'complex_usage',
        'title': '更复杂的使用',
        'description': fac.AntdParagraph(['使用组件型参数如', fac.AntdText('title', code=True), fac.AntdText('subTitle', code=True), '添加更丰富的操作'])
    },
]
    ...
    return fac.AntdSpace(
        [
            html.Div(
                [
                    html.Div(
                        getattr(views.AntdResult.demos, item['path']).render(),
                        className='demo-box',
                    ),
                    html.Div(
                        [
                    ...
                    fac.AntdTabs(
                        items=[
                            ...
                            for item in getattr(
                                views.AntdResult.demos, item['path']
                            ).code_string
                        ],
                        centered=True,

...
　　用于*为当前表格设置源数据*，每个元素为字典，其各个键名与`columns`中各个字段的`dataIndex`信息对应，并可额外设置键值对`key`作为当前行记录的唯一标识id（若未设置`key`，`AntdTable`内部会自动生成递增序号作为`key`），`data`中每条记录的字段值格式与对应字段是否开启再渲染模式，以及具体开启的再渲染模式类型有关，不同再渲染模式接受的输入值格式如下：
